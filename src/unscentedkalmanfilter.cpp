...
void UnscentedKalmanFilter::handleLidarMeasurements(const std::vector<LidarMeasurement>& dataset, const BeaconMap& map)
{
    // Assume No Correlation between the Measurements and Update Sequentially
    for(const auto& meas : dataset) {handleLidarMeasurement(meas, map);}
}

void UnscentedKalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
...
void UnscentedKalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        VectorXd state = getState();
...
void UnscentedKalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
...
Matrix2d UnscentedKalmanFilter::getVehicleStatePositionCovariance()
{
...
VehicleState UnscentedKalmanFilter::getVehicleState()
{
    if (isInitialised())
    {
...
void UnscentedKalmanFilter::predictionStep(double dt){}