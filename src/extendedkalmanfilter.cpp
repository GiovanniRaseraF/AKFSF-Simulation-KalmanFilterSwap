...
void ExtendedKalmanFilter::handleLidarMeasurements(const std::vector<LidarMeasurement>& dataset, const BeaconMap& map)
{
    // Assume No Correlation between the Measurements and Update Sequentially
    for(const auto& meas : dataset) {handleLidarMeasurement(meas, map);}
}

void ExtendedKalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
...
void ExtendedKalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        VectorXd state = getState();
...
void ExtendedKalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
...
Matrix2d ExtendedKalmanFilter::getVehicleStatePositionCovariance()
{
...
VehicleState ExtendedKalmanFilter::getVehicleState()
{
    if (isInitialised())
    {
